...
export default function PollDetailPage() {
  ...
  useEffect(() => {
    async function fetchPollAndVotes() {
      ...
      if (pollData && Array.isArray(pollData.options)) {
        const { data: voteData } = await supabase.from("votes").select("option_index").eq("poll_id", pollId);
  ...
  return (
    <Card className="max-w-xl mx-auto p-6">
      ...
      <div className="mt-6 text-sm text-muted-foreground">Total votes: {totalVotes}</div>
    </Card>
  );
}
